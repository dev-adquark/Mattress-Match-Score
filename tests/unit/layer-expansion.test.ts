...
describe('computeLayerSeparation', () => {
  ...
  it('returns 0 before progress 0.33 on home route', () => {
    expect(computeLayerSeparation('home', 0)).toBe(0);
    expect(computeLayerSeparation('home', 0.1)).toBe(0);
    expect(computeLayerSeparation('home', 0.33)).toBeCloseTo(0, 2);
  });

  it('smoothsteps from 0 to 1 between 0.33 and 0.5 on home route', () => {
    ...
    expect(sep33).toBeLessThan(0.1);
    expect(sep40).toBeGreaterThan(0.2);
    expect(sep40).toBeLessThan(0.8);
  ...
  it('smoothsteps from 1 to 0 between 0.67 and 1.0 on home route', () => {
    ...
    expect(sep80).toBeGreaterThan(0.1);
    expect(sep80).toBeLessThan(0.9);
    expect(sep100).toBeCloseTo(0, 1);
  });

  it('clamps progress outside [0, 1]', () => {
    expect(computeLayerSeparation('home', -0.5)).toBe(0);
